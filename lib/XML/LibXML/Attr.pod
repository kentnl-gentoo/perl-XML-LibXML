...
This functin is not part of DOM API. It returns attribute content in the form
...
encoding for the output string (wich is then passed to Perl as a byte string).
Otherwise the string is passed to Perl as (UTF-8 encododed) characters.
