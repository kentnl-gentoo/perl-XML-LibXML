...
Additionaly to the generic functions, that are only able to process entire
documents, XML::LibXML::SAX provides parse_chunk(). This method allows to
generate SAX events from well ballanced data such as is often provided by
databases.
